...
class TableProcessor(BlockProcessor):
    ...
    def run(self, parent, blocks):
        ...
        # add caption if the previous block matches pattern `Table: <caption text>`
        if (sibling is not None and sibling.tag == "p" and sibling.text.lower().startswith('table:')):
            caption = etree.SubElement(table, 'caption')
            caption.text = sibling.text.split(':')[1].strip()
