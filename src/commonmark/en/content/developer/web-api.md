...
**Update**: To update an existing event, the format of the payload is
the same, but the URL you are posting to must add the identifier to the
end of the URL string and the request must be PUT.
...
**Delete**: To delete an existing event, all you need is to send a
DELETE request with a identifier reference to the server you are using.

    curl -X DELETE "localhost/api/26/events/ID" -u admin:district

**Get**: To get an existing event you can issue a GET request including
the identifier like
    this:
...
of the request. Forming a GET request to the given location yields an
empty JSON response if the task has not yet completed and a JSON
taskSummary object when the task is done. Polling (conservatively) to
this resource can hence be used to wait for the task to finish.
...
We will now pretend to be the mobile user and read te message which was
...
*/api/26/interpretations* resource. A typical GET request using field
...
Several tasks which typically take a significant time to complete can be
performed asynchronously. After initiating an async task you can poll
the status through the *system/tasks* resource by suppling the task
...
A request to poll for the status of a data value import task looks like
this:
...
Tracked entity instances have full CRUD (create, read, update, delete)
support in the Web-API. Together with the API for enrollment most
operations needed for working with tracked entity instances and programs
are supported.
...
To delete a tracked entity instance you can make a request to the URL
...
Two update strategies for tracked entity instance are supported:
...
updating a new tracked entity instane.
...
patterns. You can read more about [working with TextPattern
here](#create-scorecards-pivot-table). All existing patterns will be
converted to a valid TextPattern when upgrading to 2.29.
...
To constrain the response to instances of a specific tracked entity
instance you can include a tracked entity instance query parameter, in
this case we are restricted it to available enrollments viewable for
current
    user:
...
*analytics\_completeness\_temp*, *analytics\_completenesstarget\_temp*,
...
Only the table type you select will have it's temporary table available
when the hook is run. The following table shows the valid combinations
of phase, table types and temporary tables.
