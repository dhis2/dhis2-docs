...
This version of DHIS 2 requires the following software versions to operate.
  ...
  - PostGIS database extension version 2.2 or later - required if you
    need to use the COORDINATE data type.
...
DHIS 2 user *Ehcache* as application cache provider by default. DHIS 2
can be configured to use other cache providers as well, such as
*memcached*.
...
system. A memcached server needs to be made available to DHIS 2, as it
runs as a separate service outside DHIS 2.

To use memcached as DHIS 2 application provider you can set the
following properties in the *dhis.conf* configuration file:
...
Only the *cache.provider* setting is mandatory. You need to restart DHIS
2 for the change to take effect.

You can install memcached on Ubuntu using the following command:

    sudo apt-get install memcached

Memcached will start by default after being installed and is available
on localhost port *11211*. You can start, stop and view the status of
the memcached service using the following command:
