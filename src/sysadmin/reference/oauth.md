...
| Scopes | `openid`, `profile`, `username`, `email`. These are the only scopes accepted via the admin API; anything else, including the reserved `client.*` scopes (e.g. `client.create`, used by the internal DCR registrar), is rejected with `E4000`. |
...
`clientSettings` and `tokenSettings` are omitted above, so server
defaults apply (see below).

#### Client settings: PKCE and consent { #oauth2_client_settings }

New authorization-code clients require S256 PKCE (RFC 7636) by
default:

- A client created **without** explicit `clientSettings` is saved with
  `require-proof-key: true` (S256 PKCE required on the
  authorization_code flow) and `require-authorization-consent: true`
  (consent screen shown).
- When `clientSettings` **is** supplied, it must be valid JSON and
  must contain both `settings.client.require-proof-key` and
  `settings.client.require-authorization-consent` as booleans;
  otherwise the write is rejected with `E4000`:
...
- Setting `"settings.client.require-proof-key": false` is rejected with
  `E4000`. Authorization-code clients must use S256 PKCE.
- `tokenSettings` must be valid JSON when supplied. Malformed JSON is
  rejected with `E4000` at write time.
- Already-persisted clients keep their stored settings and are not
  retroactively switched to PKCE.
...
### PKCE and consent behavior { #dcr_pkce_and_consent }

Every DCR-registered client is saved with:

- `require-authorization-consent: false`: DHIS2 overrides the Spring
  Authorization Server default so first-party device flows don't
  interrupt the user with a self-grant consent prompt.
- `require-proof-key: true`: the authorization_code flow must
  use S256 PKCE (RFC 7636). An authorization request without a
  `code_challenge`, or a token exchange with a missing or wrong
  `code_verifier`, fails with `invalid_grant`.

REST-created (non-DCR) clients default to consent **required**, so
the consent screen is shown as expected for third-party web apps
(and they require S256 PKCE too; see
[Client settings: PKCE and consent](#oauth2_client_settings)).

### Scopes { #dcr_scopes }

Omit the `scope` field from the registration payload. The server
assigns the first-party default scopes `openid profile username` to
every DCR-registered client (`email` is deliberately excluded), and
filters anything a client does send against the allowed set `openid`,
`profile`, `username`, `email`.

### Refresh token lifetime and rotation { #dcr_refresh_token_lifetime }

DCR-registered clients receive their token settings at registration
time:

- **Refresh token TTL** is taken from the `dhis.conf` key
  `oauth2.server.dcr.refresh-token-ttl` (seconds, default `2592000` =
  30 days).
- **Refresh token rotation** is enabled
  (`reuse-refresh-tokens=false`), as required for public clients by
  OAuth 2.1. Every `grant_type=refresh_token` call to `/oauth2/token`
  returns a **new** refresh token valid for the full TTL, making the
  TTL a sliding window: a device that refreshes at least once per TTL
  period stays signed in indefinitely, while an inactive device's
  session expires.

Two operational consequences:

- **Token settings are persisted per client at registration time.**
  Changing `oauth2.server.dcr.refresh-token-ttl` only affects clients
  registered after the change. Already-enrolled devices keep their
  stored settings until they re-enroll, or until an admin updates the
  client's `tokenSettings` via the OAuth2 client API.
- **Clients must persist the rotated refresh token from every refresh
  response atomically.** The superseded token is invalidated on use;
  replaying it fails with `invalid_grant`. A refresh response lost
  mid-flight therefore invalidates the session and forces
  re-authentication.
...
6. The app POSTs to `/connect/register` with payload:
   ...
   Omit the `scope` field from the payload (see
   [Scopes](#dcr_scopes)); the server assigns `openid profile username`.
...
8. Going forward, the app completes authorization_code flows against
   `/oauth2/authorize` (using S256 PKCE) and signs
   JWT client assertions (RFC 7523) with its private key when calling
   `/oauth2/token`.
...
| `user_info_response_type` | no (default `json`) | `json` (Spring's standard userinfo path) or `jwt` (signed-JWT userinfo, e.g. MOSIP eSignet). See [Signed-JWT userinfo](#oidc_signed_jwt_userinfo). |
| `user_info_jws_algorithm` | for `user_info_response_type = jwt` | JWS algorithm used to verify the signed userinfo JWT. Default `RS256`; allowed: `RS256/384/512`, `PS256/384/512`, `ES256/384/512`. |
...
### Signed-JWT userinfo (eSignet) { #oidc_signed_jwt_userinfo }

The OIDC spec allows the userinfo endpoint to return either plain
`application/json` (the default behaviour every provider in this
document uses) or a signed JWT (`application/jwt`). Some IdPs —
notably **MOSIP eSignet** — return only the signed-JWT form. Enable
the JWT path per provider by setting `user_info_response_type = jwt`:
...
- DHIS2 calls the userinfo endpoint with `Accept: application/jwt`.
- The response body is treated as a signed JWT. The signature is
  verified against the IdP's JWKS — fetched from the existing
  `jwk_uri` — using the algorithm named in `user_info_jws_algorithm`.
- The `mapping_claim` lookup then proceeds exactly as on the JSON
  path (`mapping_claim = sub` is typical for eSignet).
- The same DHIS2 user requirements apply: account must exist, be
  flagged for external authentication, and not be disabled or
  expired.

`user_info_jws_algorithm` is validated at startup against an
allow-list of asymmetric algorithms:
...
HMAC algorithms (`HS256`, `HS384`, `HS512`) and `none` are
deliberately **not** accepted — the signing key for userinfo must
come from the IdP's published JWKS, never from a shared secret.
Unknown or unsupported values cause the provider to be rejected at
startup with an explicit log line.

`user_info_response_type` is also case-insensitively validated;
the only accepted values are `json` (default) and `jwt`. Any other
value fails the provider at startup.

> **Notes**
>
> - Combining `user_info_response_type = jwt` with
>   `client_authentication_method = private_key_jwt` is supported and
>   common (eSignet typically requires both).
> - The JWKS document is fetched lazily on first login and cached
>   per provider with Nimbus's standard remote-key cache and refresh
>   policy, so a normal IdP key rotation requires no DHIS2 restart.
...
| `oauth2.server.dcr.refresh-token-ttl` | seconds | `2592000` | Refresh token TTL for DCR-registered clients (e.g. Android devices). Rotation is enabled for these clients, so the TTL is a sliding window. Applied at registration time only; see [Refresh token lifetime and rotation](#dcr_refresh_token_lifetime). |
...
**Scopes sent to `/connect/register` are rejected or not honored.**
Omit the `scope` field from the registration payload; the server
assigns `openid profile username` automatically.

**Token exchange fails with `invalid_grant` even though the code is fresh.**
The client requires S256 PKCE. Send a `code_challenge` +
`code_challenge_method=S256` on the authorization request and the
matching `code_verifier` on the token request. There is no
server-side opt-out from PKCE.

**`POST /api/oAuth2Clients` returns 409 `E4000` complaining about `settings.client.require-proof-key`.**
Supplied `clientSettings` JSON must contain both
`settings.client.require-proof-key` (must be `true`) and
`settings.client.require-authorization-consent` as booleans, or be
omitted entirely to get the server defaults. `require-proof-key:
false` is rejected.
