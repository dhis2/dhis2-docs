...
Sets the allowed list of servers to be called in relation to the [metadata pull](../developer/web-api/synchronization.md#webapi_sync_metadata_pull) functionality. It accepts comma-separated values and it's recommended that each server should end with a `/` for enhanced security. Default value is empty.
