...
| --- | --- | --- |
| contains | (expr, sub1, ...) | Searches an expression for one or more substrings. Returns true if the expression contains all the substrings. For example, the following are all true: `contains("abcd", "abcd")`, `contains("abcd", "b")` and `contains("abcd", "ab", "bc")`. Comparisons are case-sensitive. Referencing a text data element's value with this function in an indicator numerator or denominator currently causes an analytics query error rather than a computed result; use it in program indicator expressions and filters, or in program rule conditions, instead. |
| containsItems | (expr, item1, ...) | Searches an expression for one or more items. The expression is made up of comma-separated elements. `containsItems` returns true if every item exactly matches an element in the expression. For example, `containsItems("abcd", "abcd")` and `containsItems("ab,cd", "ab", "cd")` are true, but `containsItems("abcd", "b")` and `containsItems("abcd", "ab", "bc")` are false. Comparisons are case-sensitive. Use it with multi-valued data elements to check whether an item is one of the recorded values. Referencing a text data element's value with this function in an indicator numerator or denominator currently causes an analytics query error rather than a computed result, even when wrapped in `if(...)`; use it in program indicator expressions and filters, or in program rule conditions, instead. |
