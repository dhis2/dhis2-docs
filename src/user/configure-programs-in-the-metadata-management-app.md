...
| Filter | The filter is applied to the data source used for the calculation of the indicator, before the indicator expression is evaluated. The filter must evaluate to either true or false. If it evaluates to true the record is included later in the expression evaluation, if not it is ignored.<br> <br>What the filter is applied to follows the analytics type. For analytics type *event* the filter is applied to each individual event. For analytics type *enrollment* it is applied to each enrollment, with the latest event of each program stage combined on the same row, in the same way that the expression is evaluated per enrollment. The filter can, in a similar way as expressions, contain references to data elements, attributes and constants.<br> <br>The program indicator filter can in addition use logical operators. These operators can be used to form logical expressions which ultimately evaluate to either true or false. For example you can assert that multiple data elements must be a specific value, or that specific attributes must have numerical values less or greater than a constant. |
