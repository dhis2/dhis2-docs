...
| keyAnalyticsDownloadCombinationLimit | Maximum number of column or row combinations allowed when requesting analytics data in table layout, for example when downloading a pivot table from Data Visualizer. The limit is applied separately to the column combinations and to the row combinations, not to their product. Requests which exceed it fail with error code E7151 "Too many combinations of columns or rows.". Raising the limit permits larger downloads but increases memory and CPU usage on the server, as the combinations are generated in memory before the table is rendered. Default: "20000" | No |
