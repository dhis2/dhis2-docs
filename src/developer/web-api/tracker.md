...
|`order`|`String`|Field and sort direction pair in the format `field:sortDirection`.<br><br>Example: `createdAt:desc`<br><br>`field` is case-sensitive. Valid sortDirection values are `asc` and `desc`. `sortDirection` is case-insensitive and defaults to `desc`. Supported fields are `attribute`, `createdAt`, and `username`, only one at a time.|
|`filter`|`String`|Colon-separated field name with the '`eq` operator and value in the format `fieldName:eq:value`.<br><br>Example: `attribute:eq:w75KJ2mc4zz`<br><br>Filtering is supported for `attribute` and `username` fields, one at a time. Only the `eq` (equals) operator is supported.|
...
|`order`|`String`|Field and sort direction pair in the format `field:sortDirection`.<br><br>Example: `createdAt:desc`<br><br>`field` is case-sensitive. Valid sortDirection values are `asc` and `desc`. `sortDirection` is case-insensitive and defaults to `desc`. Supported fields are `createdAt`, `dataElement`, `property` and `username`, only one at a time.|
|`filter`|`String`|Colon-separated field name with the `eq` operator and value in the format `field:eq:value`.<br><br>Example: `dataElement:eq:w75KJ2mc4zz`<br><br>Filtering is supported for `property`, `dataElement` and `username` fields, one at a time. Only the `eq` (equals) operator is supported.|
